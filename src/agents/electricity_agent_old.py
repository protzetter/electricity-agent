...
A specialized AI agent for analyzing European electricity markets using real-time data 
...
try:
    ...
    print("Warning: Strands framework not available. Using basic functionality.")
    STRANDS_AVAILABLE = False
    
    # Define dummy decorators for when Strands is not available
    def tool(func):
        return func
    ...
    class Agent:
        ...
        def __call__(self, query):
            return "Strands framework not available. Please install strands-agents."
...
def initialize_model():
    """Initialize the AI model based on available configuration."""
    if not STRANDS_AVAILABLE:
        return None
    
    # Try Bedrock first
    if os.environ.get('AWS_REGION') and os.environ.get('BEDROCK_MODEL'):
        try:
            region = os.environ.get('AWS_REGION', 'us-east-1')
            model_id = os.environ.get('BEDROCK_MODEL', 'us.amazon.nova-pro-v1:0')
    ...
    logger.error("No AI model could be initialized. Please check your configuration.")
    return None
...
import numpy as np
from src.tools.entsoe_tool import get_electricity_load, get_electricity_generation, get_day_ahead_prices
...
if STRANDS_AVAILABLE and model:
...
def ask_electricity_agent(query: str):
    ...
    try:
        if not electricity_agent:
            return "Electricity agent not available. Please check your configuration and ensure Strands framework is installed."
...
if __name__ == "__main__":
    ...
    if not os.environ.get('ENTSOE_API_TOKEN'):
        print("⚠️  Warning: ENTSOE_API_TOKEN not found in environment variables.")
        print("   Please set your ENTSOE API token in config/.env")
        print()
    
    if not STRANDS_AVAILABLE:
        print("⚠️  Warning: Strands framework not available.")
        print("   Install with: pip install strands-agents")
        print()
    
    if not electricity_agent:
        print("❌ Electricity agent not available. Please check your configuration.")
        print()
        # Still allow direct tool usage
        print("You can still use the ENTSOE tools directly:")
        print("Example: get_electricity_load('DE', 24)")
        print()
    
    print("Available commands:")
    print("- Type your question about European electricity markets")
    print("- Type 'test' to run a quick test")
    print("- Type 'countries' to see supported countries")
        ...
        try:
            ...
            if user_input.lower() == "exit":
                ...
                if test_result.get('status') == 'success':
                    print(f"✅ Test successful! Got {test_result.get('total_points', 0)} data points for Germany")
                else:
                ...
                if countries_result.get('status') == 'success':
                    print("\nSupported countries:")
                    for code, name in countries_result['supported_countries'].items():
                        print(f"  {code}: {name}")
                else:
                    print("Error getting supported countries")
            elif user_input:
